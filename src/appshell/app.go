...
func NewApp() *App {
	...
	return &App{
		service: service,
		initErr: err,
		settings: &AppSettings{Pagesize: 180, ProfileID: 1, LoggedIn: false},
	}
}
...
func (a *App) SaveAppSettings(settings AppSettings) (*AppSettings, error) {
	...
	a.settings.Pagesize = settings.Pagesize
	a.settings.ProfileID = settings.ProfileID
	a.settings.LoggedIn = settings.LoggedIn
	if a.settings.Pagesize < 30 {
		a.settings.Pagesize = 30
	} else if a.settings.Pagesize > 500 {
...
func (a *App) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if zipPath == "" {
		http.Error(w, "zip source unavailable", http.StatusNotFound)
		return
	}
